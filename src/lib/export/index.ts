...
import { downloadPdfOutline } from "./outline-pdf";
...
export function exportToMarkdown(board: VidyaBoard): string {
  return serializeOutlineMarkdown(buildOutlineDocument(board));
}
...
export function downloadMarkdown(board: VidyaBoard) {
  downloadFile(
    ...
    outlineFilename(board.title, "md"),
    "text/markdown;charset=utf-8"
  );
}

export function downloadTextOutline(board: VidyaBoard) {
  downloadFile(
    `\ufeff${serializeOutlineText(buildOutlineDocument(board))}`,
    outlineFilename(board.title, "txt"),
    "text/plain;charset=utf-8"
  );
}

export function downloadHtmlOutline(board: VidyaBoard) {
  downloadFile(
    serializeOutlineHtml(buildOutlineDocument(board)),
    outlineFilename(board.title, "html"),
    "text/html;charset=utf-8"
  );
}

export { downloadPdfOutline };

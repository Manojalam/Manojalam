...
const USER_MESSAGES: Record<ExportErrorCode, string> = {
  ...
  INVALID_SCALE: "Choose a valid PNG, JPG, or PDF export scale greater than zero.",
  ...
  REMOTE_FONT_CORS: "A remote font could not be embedded. PNG, JPG, and PDF export will use a browser-safe fallback font.",
  ...
  JPEG_BLOB_CREATION_FAILED: "JPG encoding failed.",
...
export function classifyExportError(stage: ExportStage, error: unknown): ExportErrorCode {
  ...
  switch (stage) {
    ...
    case "encode-jpg":
      return "JPEG_BLOB_CREATION_FAILED";
